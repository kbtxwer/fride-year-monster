...
function init() {
	...
	while(true){
		...
		//执行一次收爆竹大约耗时40分钟，相比之下执行常规任务的耗时可以忽略
		repeatTimes = 27;
		//执行收爆竹
		while(repeatTimes-- > 0){
			regularTask();
...
function regularTask(noCollect){
	...
	if (text("爆竹满了~~").exists()) {
		...
		CustomSleep(30*60, 45*60, '随机等待 30-45 min 后收集爆竹');
    ...
    for (let index = 0; index < allSelector.length; index++) {
        for (var i = 0; i < TASK_LIST.length; i++) {
            ...
            if (list.size() > 0) {
                ...
                isBackFlag = (TASK_LIST[i].indexOf("浏览可得") >= 0 || TASK_LIST[i].indexOf("浏览并关注") >= 0) ? true : false;
